...
const Dashboard: React.FC = () => {
    ...
    const fetchBalance = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:5026/api/BillingAccount/GetBillingAccountByMail/${encodeURIComponent(
            email
          )}`,
          {
            headers: {
    ...
    scanpay: (
      <div className="dashboard-section">
        ...
        >
          <h5>📱 Scan and Pay</h5>
          <div className="d-grid gap-2">
            <button className="btn btn-info" onClick={() => navigate("/qr")}>
              Scan a Colleague's QR
            </button>
            <button
              className="btn btn-secondary"
              onClick={() => navigate("/payment-options")}
            >
              Pay Someone's Coffee
            </button>
            <button
  ...
  return (
    ...
    >
      ...
      {/* Filter Tabs */}
      <div
        className={`py-3 px-3 sticky-top ${
          isDesktop
            ? "justify-content-center d-flex"
            : "overflow-auto d-flex gap-2"
        }`}
        style={{ top: "84px", zIndex: 10, backgroundColor: "#1D1D1B" }}
      >
        {["All", "Friends", "Tuition", "Food", "Parking", "Scan & Pay"].map(
          (label) => {
            const key = label.toLowerCase().replace(/ & | /g, "");
            return (
              <button
                key={key}
                className={`btn btn-sm mx-1 filter-tab-btn ${
                  activeFilter === key ? "btn-warning" : "btn-secondary"
                }`}
      ...
      <div
        className={`dashboard-scroll ${
          isDesktop ? "px-5 pt-4" : "px-3 pt-3"
        } flex-grow-1 overflow-auto`}
      >
        <div className="card bg-dark text-white mb-3 p-3 border border-info">
          <h5 className="text-info">🎫 My QR Code</h5>
          <p>
            Show your QR code so others can pay you directly or add you as a
            friend.
          </p>
          <button className="btn btn-info" onClick={() => navigate("/user-qr")}>
            Show QR Code
          </button>
        </div>
